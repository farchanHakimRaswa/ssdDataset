...
def extract_coor(txt_file, img_width, img_height):
   ...
   # x_min_rect = (2 * x_rect_mid * img_width - width_rect * img_width) \
   #     / 2
   # x_max_rect = (2 * x_rect_mid * img_width + width_rect * img_width) \
   #    / 2
   # y_min_rect = (2 * y_rect_mid * img_height - height_rect
   #               * img_height) / 2
   # y_max_rect = (2 * y_rect_mid * img_height + height_rect
   #               * img_height) / 2

    return (x_rect_mid, y_rect_mid, width_rect, height_rect)
...
for line in fw:
    ...
    for ii in range(len(txt_file)):
        ...
        (x_min_rect, x_max_rect, y_min_rect, y_max_rect) = \
            extract_coor(txt_file[ii], img_width, img_height)
    ...
    file_output = etree.tostring(root, pretty_print=True,
                                 encoding='UTF-8')
  
    ff = open('%s%s.xml' % (save_path, img_name[:-4]), 'w',
              encoding='utf-8')
